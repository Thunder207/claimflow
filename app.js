...
function insertDefaultData() {
    ...
    db.run(`INSERT OR IGNORE INTO app_settings (key, value) VALUES ('phone_plan_max', '50.00')`, (err) => {
        if (err && !err.message.includes('UNIQUE constraint failed')) {
            console.error('❌ Error inserting phone_plan_max:', err.message);
        }
    });
    db.run(`INSERT OR IGNORE INTO app_settings (key, value) VALUES ('phone_device_max', '50.00')`, (err) => {
        if (err && !err.message.includes('UNIQUE constraint failed')) {
            console.error('❌ Error inserting phone_device_max:', err.message);
        }
    });
...
app.get('/api/settings/phone', requireAuth, (req, res) => {
    db.all(`SELECT key, value FROM app_settings WHERE key IN ('phone_plan_max', 'phone_device_max', 'phone_claim_window')`, [], (err, rows) => {
        if (err) return res.status(500).json({ error: 'Database error' });
        const settings = {};
        rows.forEach(r => settings[r.key] = r.value);
        res.json({
            plan_max: parseFloat(settings.phone_plan_max || '50.00'),
            device_max: parseFloat(settings.phone_device_max || '50.00'),
...
app.put('/api/settings/phone', requireAuth, requireRole('admin'), (req, res) => {
    const { plan_max, device_max, claim_window } = req.body;
    
    // Get old values for audit
    db.all(`SELECT key, value FROM app_settings WHERE key IN ('phone_plan_max', 'phone_device_max', 'phone_claim_window')`, [], (err, oldRows) => {
        const oldSettings = {};
        if (oldRows) oldRows.forEach(r => oldSettings[r.key] = r.value);
        
        if (plan_max !== undefined) {
            db.run(`INSERT OR REPLACE INTO app_settings (key, value, updated_by, updated_at) VALUES ('phone_plan_max', ?, ?, CURRENT_TIMESTAMP)`, [String(plan_max), req.user.employeeId]);
        }
        if (device_max !== undefined) {
            db.run(`INSERT OR REPLACE INTO app_settings (key, value, updated_by, updated_at) VALUES ('phone_device_max', ?, ?, CURRENT_TIMESTAMP)`, [String(device_max), req.user.employeeId]);
        }
        if (claim_window !== undefined) {
            db.run(`INSERT OR REPLACE INTO app_settings (key, value, updated_by, updated_at) VALUES ('phone_claim_window', ?, ?, CURRENT_TIMESTAMP)`, [String(claim_window), req.user.employeeId]);
        }
        
        // Audit log
        db.run(`INSERT INTO settings_audit_log (setting_key, old_value, new_value, changed_by) VALUES (?, ?, ?, ?)`,
            ['phone_benefit_settings',
             `Plan: $${oldSettings.phone_plan_max || '50.00'}, Device: $${oldSettings.phone_device_max || '50.00'}, Window: ${oldSettings.phone_claim_window || '2'}`,
             `Plan: $${plan_max || oldSettings.phone_plan_max || '50.00'}, Device: $${device_max || oldSettings.phone_device_max || '50.00'}, Window: ${claim_window || oldSettings.phone_claim_window || '2'}`,
             req.user.employeeId]);
...
app.post('/api/phone-claims', requireAuth, upload.array('receipts', 30), (req, res) => {
    try {
        ...
        // Get settings
        db.all(`SELECT key, value FROM app_settings WHERE key IN ('phone_plan_max', 'phone_device_max')`, [], (err, settingRows) => {
            if (err) return res.status(500).json({ error: 'Database error' });
            const settings = {};
            settingRows.forEach(r => settings[r.key] = r.value);
            const planMax = parseFloat(settings.phone_plan_max || '50.00');
            const deviceMax = parseFloat(settings.phone_device_max || '50.00');
            ...
            function processNextClaim(idx) {
                ...
                const planClaim = Math.min(planReceipt, planMax);
                const deviceClaim = Math.min(deviceReceipt, deviceMax);
